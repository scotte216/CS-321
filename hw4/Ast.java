...
class Ast {
  ...
  public static class ClassDecl extends Node {
    ...
    void checkVarInit(VarSet initSet) throws Exception {
        ...
        for (VarDecl field : flds)
        {
           field.checkVarInit(initSet);
           initSet = initSet.add(initSet, field.nm);
        }
        //System.out.println("Initset: " + initSet);
  ...
  public static class MethodDecl extends Node {
    ...
    boolean checkReach(boolean reachable) throws Exception {
          for (Stmt s: stmts)
          {
            // System.out.println("Statment: " + s + "Reachable: " + reachable);
             reachable = s.checkReach(reachable) && reachable;
          }
          return true;
    }
    ...
    void checkVarInit(VarSet initSet) throws Exception {
        for (Param param: params)
            initSet = initSet.add(initSet, param.nm);

        for (VarDecl var: vars)
        {
            var.checkVarInit(initSet);
            if (var.init != null)
                initSet = initSet.add(initSet,var.nm);
        }
       // System.out.println("Initset: " + initSet);

        for (Stmt st: stmts)
            initSet = st.checkVarInit(initSet);
    }
  }
  ...
  public static class Block extends Stmt {
    ...
    public VarSet checkVarInit(VarSet initSet) throws Exception
    {
        for (Stmt s: stmts)
  ...
  public static class Assign extends Stmt {
    ...
    public VarSet checkVarInit(VarSet initSet) throws Exception
    {
        ...
        return initSet.add(initSet,lhs.toString());        
    }
...
//System.out.println("Initset: " + initSet);
  ...
  public static class If extends Stmt {
    ...
    public VarSet checkVarInit(VarSet initSet) throws Exception {
        ...
        VarSet ifSet = s1.checkVarInit(initSet);
        VarSet elseSet = null;
        if ( s2 != null)
            elseSet = s2.checkVarInit(initSet);
//System.out.println("initSet: " + initSet);
//System.out.println("ifSet: " + ifSet);
//System.out.println("elseSet: " + elseSet);
        if (elseSet == null)
            return initSet;
        else
            return initSet.union(initSet,ifSet.intersect(ifSet,elseSet));
    }
    
    boolean checkReach(boolean reachable) throws Exception {
      if (!reachable) 
          throw new StaticError("Unreachable statement: " + this);
      boolean ifClause = s1.checkReach(reachable);
      boolean elseClause;
      if (s2 != null) 
          elseClause = s2.checkReach(reachable);
      else
          elseClause = false;
      

      return ifClause || elseClause;
    }
  ...
  public static class While extends Stmt {
    ...
    boolean checkReach(boolean reachable) throws Exception {
      ...
      return s.checkReach(reachable) && reachable;
    }
  ...
  public static class Print extends Stmt {
    ...
    boolean checkReach(boolean reachable) throws Exception {
      //System.out.println("pReachable: " + reachable);
      if (!reachable) 
          throw new StaticError("Unreachable statement: " + this);
      return true;
    }
  ...
  public static class Binop extends Exp {
    ...
    public void checkVarInit(VarSet initSet) throws Exception {
  ...
  public static class ArrayElm extends Exp {
    ...
    public void checkVarInit(VarSet initSet) throws Exception {
 //       System.out.println("Array's initSet: " + initSet);
  ...
  public static class Id extends Exp {
    ...
    public void checkVarInit(VarSet set) throws Exception {
//        System.out.println("ID's VarSet: " + set);
        if (!set.contains(this.toString()))
        {
